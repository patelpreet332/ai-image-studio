...
from ldm_patched.ldm.flux.layers import EmbedND
from ldm_patched.ldm.flux.math import apply_rope
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        e = (ldm_patched.modules.model_management.cast_to(self.modulation, dtype=x.dtype, device=x.device) + e).chunk(6, dim=1)
...
class Head(nn.Module):
    ...
    def forward(self, x, e):
        ...
        """
        # assert e.dtype == torch.float32
        e = (ldm_patched.modules.model_management.cast_to(self.modulation, dtype=x.dtype, device=x.device) + e.unsqueeze(1)).chunk(2, dim=1)
...
class MLPProj(torch.nn.Module):
    ...
    def forward(self, image_embeds):
        if self.emb_pos is not None:
            image_embeds = image_embeds[:, :self.emb_pos.shape[1]] + ldm_patched.modules.model_management.cast_to(self.emb_pos[:, :image_embeds.shape[1]], dtype=image_embeds.dtype, device=image_embeds.device)
...
class WanModel(torch.nn.Module):
    ...
    def forward(self, x, timestep, context, clip_fea=None, time_dim_concat=None, transformer_options={}, **kwargs):
        bs, c, t, h, w = x.shape
        x = ldm_patched.ldm.common_dit.pad_to_patch_size(x, self.patch_size)
        ...
        if time_dim_concat is not None:
            time_dim_concat = ldm_patched.ldm.common_dit.pad_to_patch_size(time_dim_concat, self.patch_size)
